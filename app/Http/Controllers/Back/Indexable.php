...
trait Indexable
{
    ...
    public function index(Request $request)
    {
        ...
        $links = $records->appends ($parameters)->links (env('THEME').'.back.pagination');

        // Ajax response
        if ($request->ajax ()) {
            return response ()->json ([
                'table' => view (env('THEME').".back.$this->table.table", [$this->table => $records])->render (),
                'pagination' => $links->toHtml (),
            ]);
        }

        return view (env('THEME').".back.$this->table.index", [$this->table => $records, 'links' => $links]);
    }
